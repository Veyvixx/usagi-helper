...
export default {
  ...
  async execute(message: Message, args: string[]) {
    ...
    const db = getDb();
    db.prepare("UPDATE guild_config SET mute_role_id = ? WHERE guild_id = ?").run(role.id, message.guild!.id);

    await message.reply({ embeds: [embed("✅ Mute Role Set", `Mute role is now ${role}.`)] });
  },
};
